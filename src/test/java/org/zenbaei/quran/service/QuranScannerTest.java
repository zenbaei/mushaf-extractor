...
public class QuranScannerTest extends BaseTest {
	...
	public void test_ayah_range_from_fateha_page_should_be_1_to_7() {
		...
		final String surahName = ArabicUtils.normalizeEnhanced(ayahs.get(0).surahName.get());
	...
	public void test_ayah_range_from_page_with_multiple_surah() {
		...
		final String exptectedSurahName1 = ArabicUtils.normalizeEnhanced(ayahRanges.get(0).surahName.get());
		final String exptectedSurahName2 = ArabicUtils.normalizeEnhanced(ayahRanges.get(1).surahName.get());
		final String exptectedSurahName3 = ArabicUtils.normalizeEnhanced(ayahRanges.get(2).surahName.get());
	...
	public void test_ayah_range_from_page_with_no_surah() {
		...
		assertThat(ayahRanges.get(0).surahName.isPresent(), is(false));
	}
	...
	public void test_ayah_range_from_page_with_surah_as_last_line_should_have_zero_value_but_surah_name_should_be_present() {
		...
		assertThat(ayahRanges.get(1).surahName.isPresent(), is(true));

		final String exptectedSurahName1 = ArabicUtils.normalizeEnhanced(ayahRanges.get(1).surahName.get());
	...
	public void test_surah_count_should_be_114() {
		...
		for (int i = 0; i < pages.size(); i++) {
			...
			ranges.stream()
				.filter(ay -> ay.surahName.isPresent())
				.forEach(ay -> surahNames.add(ay.surahName.get()));
		}
